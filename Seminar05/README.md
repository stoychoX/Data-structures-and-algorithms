...
Миналия семинар работихме с един класически начин да представим свързан списък, а именно просто чрез указател към първия му елемент. Днес ще разгледаме представянето на свързания списък като клас. 
Разбира се, какво може да е толкова трудното на това да представим свързания списък като клас? Вместо функция *freeLinkedList* ще имаме деструктор, всички нужни функции ще ги направим като член-функции на новосъздадения ни клас и готово.
...
* Също така пазим още една допълнителна променлива от тип size_t за да пазим дължината на списъка.
...
Ще направим итератора **вътрешен клас за класът LinkedList**, който ще е **публичен**.
```cpp
// ...
public:
class Iterator {
...
/// ...
```
Вече сме готови да проверим дали нашия метод работи.
```cpp
int main() {
    LinkedList<int> ll;
...
```
В случая, въпреки че ll е const с отново променяме стойността му. 
...
## auto
Най - вероятно вече сте чували за ключовата дума *auto*. Чрез нея карате компилатора сам да прецени какъв тип да използва за променливата. Тя е удобна когато имате дълги типове. Прави кодът по - четим и разбираем. Но като всяко хубаво нещо **с auto не е добре да се прекалява.**
