...
vector<T, Allocator>::vector(size_t count, const T& initial)
	...
	, _capacity(count)
{
	_allocator.construct_range(_data, _data + _size, initial);
}
...
void vector<T, Allocator>::copy(const vector<T, Allocator>& other)
{
	...
	_allocator.copy_range(_data, other._data, other._data + other.size());
...
{
	_allocator.destroy_range(_data, _data + size());
...
void vector<T, Allocator>::resize(size_t n)
{
	if (n < size())
	{
		_allocator.destroy_range(_data + n, _data + size());
		_size = n;
	}
	else if (n > size())
	{
		if (n <= capacity())
		{
			_allocator.construct_range(_data + size(), _data + n);
			_size = n;
		}
		else
		{
			...
			// TODO: what if range is not movable?
			// _allocator.copy_range(new_data, _data, _data + size());
			_allocator.move_range(new_data, _data, _data + size());
			_allocator.construct_range(new_data + size(), new_data + n);
...
void vector<T, Allocator>::erase(const_iterator first, const_iterator last)
{
	...
	if (last != c_end())
	{
		_allocator.move_range(_data + begin_offset, _data + end_offset, _data + size());
	}
	_allocator.destroy_range(_data + end_offset, _data + size());
	_size -= deleted_count;
}
